...
T** new_Array2D(int row,int col){
    ...
    //申请内存，sizeof(T*)*row ：存放row个行指针
    T **arr=(T **)malloc(point_size*row+size*row*col);
    if(arr!=NULL){
        T *head=(T*)(arr+point_size*row);
        for (int i = 0; i < row; i++)
        {
            /* code */
            arr[i]=(T*)(head+i*col*size);
            for (int j = 0; j < col; j++)
            {
                /* code */
                new(&arr[i][j]) T;
...
void parse_python_viterbi_data_opration(){
    ...
    const int alignment_rows=PyArray_DIM(alignment_matrix,0);
    const int alignment_cols=PyArray_DIM(alignment_matrix,1);
    ...
    const int traceback_mr_rows=PyArray_DIM(traceback_matrix_rows,0);
    const int traceback_mr_cols=PyArray_DIM(traceback_matrix_rows,1);
    ...
    const int traceback_mc_rows=PyArray_DIM(traceback_matrix_cols,0);
    const int traceback_mc_cols=PyArray_DIM(traceback_matrix_cols,1);
    ...
    time(&start);
    //下面完成viterbi主要算法
    //采用动态申请内存的方法在传出数据时出现类型不匹配问题
    double alignment_matrix_data[alignment_rows][alignment_cols]={};
    ...
    //vector< vector<int> > traceback_matrix_rows_data(traceback_mr_rows);
    //int** traceback_matrix_rows_data=new int*[traceback_mr_rows];
    //申请连续内存
    int *mr_arr=new int[traceback_mr_rows*traceback_mr_cols];
    int (*traceback_matrix_rows_data)[traceback_mr_cols]=(int(*)[traceback_mr_cols])mr_arr;
    for (int i = 0; i < traceback_mr_rows; i++)
    {
        /* code */
        //traceback_matrix_rows_data[i]=new int[traceback_mr_cols];
        //traceback_matrix_rows_data[i].resize(traceback_mr_cols);
    ...
    //vector< vector<int> > traceback_matrix_cols_data(traceback_mc_rows);
    //int** traceback_matrix_cols_data=new int*[traceback_mc_rows];
    //申请连续内存
    int *mc_arr=new int[traceback_mc_rows*traceback_mc_cols];
    int (*traceback_matrix_cols_data)[traceback_mc_cols]=(int(*)[traceback_mc_cols])mc_arr;
    for (int i = 0; i < traceback_mc_rows; i++)
    {
        /* code */
        //traceback_matrix_cols_data[i]=new int[traceback_mc_cols];
        //traceback_matrix_cols_data[i].resize(traceback_mc_cols);
    ...
    //示例数组
    double example_array[2][2]={{1,2},{3,4}};
    npy_intp example_dims[2]={2,2};
    PyObject* PyArray_example=PyArray_SimpleNewFromData(2,example_dims,NPY_DOUBLE,example_array);
    ...
    // PyObject* PyArray_traceback_row=PyArray_SimpleNewFromData(2,traceback_matrix_row_dims,NPY_INT,&traceback_matrix_rows_data[0][0]);
    // PyObject* PyArray_traceback_col=PyArray_SimpleNewFromData(2,traceback_matrix_col_dims,NPY_INT,&traceback_matrix_cols_data[0][0]);
    PyObject* PyArray_traceback_row=PyArray_SimpleNewFromData(2,traceback_matrix_row_dims,NPY_INT32,&traceback_matrix_rows_data[0][0]);
    PyObject* PyArray_traceback_col=PyArray_SimpleNewFromData(2,traceback_matrix_col_dims,NPY_INT32,&traceback_matrix_cols_data[0][0]);

    //定义与python函数参数个数的pytuple对象
    PyObject* ArgArray1 =PyTuple_New(3);
    PyObject* ArgArray2=PyTuple_New(2);
    PyTuple_SetItem(ArgArray1,0,PyArray_alignment);
    PyTuple_SetItem(ArgArray1,1,PyArray_traceback_row);
    PyTuple_SetItem(ArgArray1,2,PyArray_traceback_col);

    // PyObject* pDict=PyModule_GetDict(pModule);
    // PyObject* pPythonFunc=PyDict_GetItemString(pDict,"parse_cpp_return_data");
    
    PyObject* pPythonFunc1=PyObject_GetAttrString(pModule,"parse_cpp_return_data");
    //PyObject* pPythonFunc2=PyObject_GetAttrString(pModule,"parse_cpp_return_data2");

    PyObject_CallObject(pPythonFunc1,ArgArray1);
    //PyObject_CallObject(pPythonFunc2,ArgArray2);
